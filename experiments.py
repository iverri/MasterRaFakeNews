...
def run_recommender_comparison_experiment(
    ...
    model_data = results_df[
        [col for col in model_vars if col in results_df.columns]
    ]
    ...
    for recommender in [type.value for type in RecommenderType]:
        ...
        recommender_data = model_data[
            ...
            (model_data.groupby(["RunId", "iteration"])["Step"].transform("max") == model_data["Step"])
        ]
...
if __name__ == "__main__":
    ...
    results_df, model_data, summary_df = run_recommender_comparison_experiment(
        ...
        max_steps=120,       # Steps per run
        n_agents=100,       # Number of agents
        m_links=5,         # Links per new node
        news_amount=300,    # Initial news items
        fake_news_percentage=10,  # Percentage of fake news
        bot_percentage=5,   # Percentage of bots
        influencer_percentage=5,  # Percentage of influencers
