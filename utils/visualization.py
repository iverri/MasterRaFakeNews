...
def visualize_network(network, agent_types=None):
    ...
    # Draw the network
    nx.draw_networkx_nodes(network, pos, node_color=node_colors, node_size=node_sizes)
...
def create_metrics_line_chart(model, metric_names, title):
    ...
    if "Misinformation" in title:
        ...
        y_max = max(
            max(model.datacollector.model_vars[metric_names[0]]) * 100 * 1.1, 20
        )
...
def create_cluster_content_similarity_plot(model):
    ...
    df = model.datacollector.get_model_vars_dataframe()
    if (
        "Within_Cluster_Content_Similarity" not in df
        or "Between_Cluster_Content_Similarity" not in df
    ):
        return None
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(
        df["Within_Cluster_Content_Similarity"],
        label="Within-Cluster Similarity",
        color="blue",
    )
    ax.plot(
        df["Between_Cluster_Content_Similarity"],
        label="Between-Cluster Similarity",
        color="orange",
    )
...
def ClusterContentSimilarityDashboard(model):
    ...
    if fig is not None:
        ...
        solara.Markdown(
            "No data available yet. Run the simulation to see cluster content similarity."
        )
...
def create_echo_chamber_strength_plot(model):
    ...
    df = model.datacollector.get_model_vars_dataframe()
    if (
        "Echo_Chamber_Strength_Diff" not in df
        or "Echo_Chamber_Strength_Ratio" not in df
    ):
        return None
    fig, ax1 = plt.subplots(figsize=(8, 4))
    ax1.plot(
        df["Echo_Chamber_Strength_Diff"],
        label="Difference (Within - Between)",
        color="purple",
    )
    ax1.set_xlabel("Step")
    ...
    ax2 = ax1.twinx()
    ax2.plot(
        df["Echo_Chamber_Strength_Ratio"],
        label="Ratio (Within / Between)",
        color="green",
    )
...
def EchoChamberStrengthDashboard(model):
    ...
    if fig is not None:
        ...
        solara.Markdown(
            "No data available yet. Run the simulation to see echo chamber strength."
        )
...
def create_average_diversity_plot(model):
    ...
    ax.plot(
        df["Average_Diversity_Score"], marker="o", color="#9370db"
    )  # Medium purple color
...
def AverageDiversityDashboard(model):
    ...
    if fig is not None:
        ...
        solara.Markdown(
            "No diversity data available yet. Run the simulation to see average diversity scores."
        )
...
def create_visualization(model_class):
    ...
    viz = SolaraViz(
        model=model, components=[DashboardLayout], model_params=model_params
    )
    return viz
