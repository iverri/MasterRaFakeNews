...
def setup_datacollector(model):
    ...
    model_reporters = {
        ...
        "Average_Feed_Size": lambda m: (
            sum(len(a.recommended_content) for a in m.agents if hasattr(a, "recommended_content"))
            / len(m.agents) if len(m.agents) > 0 else 0
        ),

        "Average_Diversity_Score": lambda m: (
            sum(a.diversity_score for a in m.agents if hasattr(a, "diversity_score"))
            / len(m.agents) if len(m.agents) > 0 else 0
        ),
    ...
    if getattr(model, "collect_community_data", False):
        ...
        model_reporters["Number_Of_Communities"] = lambda m: (
            ...
            if hasattr(m, "community_data") and "communities" in m.community_data
            else 0
        )
    ...
    agent_reporters = {
        ...
        "Misinformation_In_Recommendations": lambda a: (
            sum(1 for c in getattr(a, "recommended_content", []) if getattr(c, "isFake", False))
        ),
    }
