...
from utils.metrics import matrix_cosine_similarity, vec_mat_cosine_similarity
import numpy as np
...
def calculate_diversity(topic_vectors):
    ...
    similarity_matrix = matrix_cosine_similarity(vectors)
...
def guaranteed_diversity_reranking(user_preferences, recs, k=10, pre_calculated=None):
    ...
    if (
        pre_calculated
        and "relevance_scores" in pre_calculated
        and len(pre_calculated["relevance_scores"]) == len(recs)
    ):
        relevance_scores = pre_calculated["relevance_scores"]
    else:
        relevance_scores = vec_mat_cosine_similarity(
            user_preferences, rec_topic_vectors
        )
    ...
    if new_diversity <= original_diversity:
        ...
        pairwise_distances = 1 - matrix_cosine_similarity(rec_topic_vectors)
...
def diversity_reranking(
    user_preferences, recs, k=10, pre_calculated=None, diversity_level=0.5
):
    """
    ...
    if (
        pre_calculated
        and "relevance_scores" in pre_calculated
        and len(pre_calculated["relevance_scores"]) == n_items
    ):
        relevance_scores = pre_calculated["relevance_scores"]
    else:
        relevance_scores = vec_mat_cosine_similarity(
    ...
    if pre_calculated and "similarity_matrix" in pre_calculated:
        ...
        similarity_matrix = matrix_cosine_similarity(rec_topic_vectors)
            ...
            if best_idx is None:
                ...
                if best_idx is None and len(remaining_indices) > 0:
                    best_idx = remaining_indices[
                        np.argmax(relevance_scores[remaining_indices])
                    ]
